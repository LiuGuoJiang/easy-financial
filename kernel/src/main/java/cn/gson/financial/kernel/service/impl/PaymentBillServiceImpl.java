...
public class PaymentBillServiceImpl extends ServiceImpl<PaymentBillMapper, PaymentBill> implements PaymentBillService {
    ...
    @Override
    public boolean save(PaymentBill entity) {
    ...
    public void audit(Integer accountSetsId, Integer id, UserVo user, Boolean generateVoucher) {
        PaymentBill bill = getBill(accountSetsId, id);
        if ("AUDITED".equals(bill.getStatus())) {
            throw new ServiceException("付款单已审核！");
        }
        ...
        flow.setFlowNo("FK" + bill.getBillNo());
        ...
        flow.setSourceType("PAYMENT");
        flow.setSourceId(bill.getId());
        flow.setVoucherId(voucherId);
        flow.setCounterparty(bill.getPayee());
    ...
    public void cancelAudit(Integer accountSetsId, Integer id) {
        PaymentBill bill = getBill(accountSetsId, id);
        if (!"AUDITED".equals(bill.getStatus())) {
            throw new ServiceException("付款单未审核！");
        }
        checkVoucher(bill.getVoucherId());
        LambdaQueryWrapper<FundFlow> flowQw = Wrappers.lambdaQuery();
        flowQw.eq(FundFlow::getAccountSetsId, accountSetsId).eq(FundFlow::getSourceType, "PAYMENT").eq(FundFlow::getSourceId, id);
        FundFlow flow = fundFlowMapper.selectOne(flowQw);
        if (flow != null && Boolean.TRUE.equals(flow.getReconciled())) {
            throw new ServiceException("付款流水已对账，不能反审核！");
        }
        if (flow != null) {
    ...
    private PaymentBill getBill(Integer accountSetsId, Integer id) {
        PaymentBill bill = baseMapper.selectById(id);
    ...
    private Integer createVoucher(PaymentBill bill, UserVo user) {
        FundAccount account = fundAccountMapper.selectById(bill.getAccountId());
        ...
        voucher.setDetails(Arrays.asList(detail(categorySubject, bill.getSummary(), bill.getAmount(), null), detail(accountSubject, bill.getSummary(), null, bill.getAmount())));
